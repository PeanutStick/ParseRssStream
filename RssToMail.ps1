...
$NbJours = -9# Si on lance le script toutes les semaines, tu mets 7 pour avoir les infos des 7 derniers jours.
$From = "exemple@mail.com"
$to = "exemple@mail.com"

$Smtp = "the SMTP server"
$test1 = Test-Path $fichier1 # Si le fichier n'existe pas, je le crée.
...
$_.published -ge $(get-date).AddDays($NbJours)# Pour avoir les infos de la semaine. Il est possible de mettre -1 pour n'avoir que la journée.
}
do{
...
do{
    ...
    $lastdate = (Get-Date $lastdate) # Vu que c'est un fichier texte il y a un retour à la ligne, donc dans startdate[0] il y a la date, startdate[1] est vide
